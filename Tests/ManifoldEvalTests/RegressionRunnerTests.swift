...
final class RegressionRunnerTests: XCTestCase {
  ...
  func testMovedAcrossLegs() async throws {
    ...
    let outcome = try await RegressionRunner.run(
      ...
      scorer: scorer,
      captureBaseline: {
        await log.record("baseline")
        return self.makeRun(output: "the answer is 4")
      },
      captureReDriven: {
        await log.record("redriven")
        return self.makeRun(output: "I'm not sure")
      }
    )
    ...
    XCTAssertEqual(
      order, ["baseline", "redriven"], "baseline must be captured before the re-drive")
  }
  ...
  func testUnscorableBaselineShortCircuitsWithoutReDrive() async throws {
    ...
    let outcome = try await RegressionRunner.run(
      ...
      captureBaseline: {
        await log.record("baseline")
        return self.makeRun(output: "anything")
      },
      captureReDriven: {
        await log.record("redriven")
        return self.makeRun(output: "anything")
  ...
  func testPromptMismatchIsIndeterminate() async throws {
    let outcome = try await RegressionRunner.run(
      ...
      scorer: SubstringRegressionScorer(expected: "4"),
      captureBaseline: {
        self.makeRun(output: "4", promptSha256: String(repeating: "a", count: 64))
      },
      captureReDriven: {
        self.makeRun(output: "4", promptSha256: String(repeating: "b", count: 64))
      }
    )
    switch outcome.verdict {
    case .indeterminate(let reason):
