...
final class CLILaneRunnerSmokeTests: XCTestCase {
  ...
  func testIFEvalSmokeWithSyntheticResponses() throws {
    ...
    XCTAssertEqual(
      result.score.totalCases, cases.count,
    ...
    XCTAssert(
      result.markdown.contains("IFEval Results"),
      "report must have '# IFEval Results' header")
    XCTAssert(
      result.markdown.contains("Strict Accuracy"),
      "report must contain 'Strict Accuracy' row")
    XCTAssert(
      result.markdown.contains("Total Cases"),
  ...
  func testBFCLSmokeWithEmptyResponses() async throws {
    ...
    XCTAssert(
      markdown.contains("BFCL Results"),
      "report must have '# BFCL Results' header")
    XCTAssert(
      markdown.contains("Overall:"),
  ...
  func testBFCLSmokeWithPrecomputedResponses() async throws {
    ...
    let simple = try XCTUnwrap(
      result.categoryResults.first { $0.category == .simple },
  ...
  func testMTEBRenderMarkdownStructure() {
    ...
    XCTAssert(
      markdown.contains("MTEB-STS Results"),
  ...
  func testMTEBLoadBuiltinFixture() throws {
    ...
    XCTAssert(
      label.lowercased().contains("scaffold") || label.lowercased().contains("built-in"),
