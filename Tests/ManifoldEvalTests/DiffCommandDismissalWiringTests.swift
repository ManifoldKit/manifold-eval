...
final class DiffCommandDismissalWiringTests: XCTestCase {
    ...
    private func genuineDivergenceOutcome(
        ...
        let ollamaRuns = (0..<2).map {
            rawRun(backend: "ollama", model: ollamaModel, promptSha: promptSha, output: ollamaOutput, inputTokens: tokens, repeatIndex: $0)
        }
        let llamaRuns = (0..<2).map {
            rawRun(backend: "llama.cpp", model: "\(ollamaModel).gguf", promptSha: promptSha, output: llamaOutput, inputTokens: tokens, repeatIndex: $0)
        }
        ...
        XCTAssertEqual(comparison.divergence, .genuineDivergence, "fixture must produce the state under test", file: file, line: line)
        return DifferentialOutcome(promptSha256: promptSha, ollama: ollamaReport, llama: llamaReport, comparison: comparison)
    }
    ...
    private func extractField(_ name: String, from report: String, file: StaticString = #filePath, line: UInt = #line) throws -> String {
        let marker = "- \(name): `"
    ...
    func testLiveDismissalSuppressesGenuineDivergenceFromDiff() throws {
        let outcome = try genuineDivergenceOutcome(ollamaOutput: "The answer is 4.", llamaOutput: "The answer is five.")

        // Baseline: no --dismissals flag at all -> unchanged, flagged.
        let noFlag = DiffCommand.evaluate(outcome: outcome, ollamaModel: "modelA", dismissalsPath: nil, warn: { _ in })
        XCTAssertEqual(noFlag.exitCode, 1)
        XCTAssertTrue(noFlag.report.contains("not checked"), "absent --dismissals must not consult any ledger")

        // Round 1 with an empty ledger at the path: still surfaces (exit 1), but
        // the report must expose a copyable cell + signature.
        let url = ledgerURL()
        let empty = DiffCommand.evaluate(outcome: outcome, ollamaModel: "modelA", dismissalsPath: url.path, warn: { _ in })
        ...
        ledger.record(finding, reason: "confirmed by-design: renderer wording", recordedAt: Date(), ttl: 3600)
        try ledger.save(to: url)

        // Round 2: the SAME divergence must now be suppressed.
        let suppressed = DiffCommand.evaluate(outcome: outcome, ollamaModel: "modelA", dismissalsPath: url.path, warn: { _ in })
        XCTAssertEqual(suppressed.exitCode, 0, "a live dismissal for this exact (cell, signature) must suppress genuineDivergence")
        XCTAssertTrue(suppressed.report.contains("SUPPRESSED"), "the report must be annotated as suppressed")
        ...
        let differentCell = DiffCommand.evaluate(outcome: outcome, ollamaModel: "modelB-unrelated", dismissalsPath: url.path, warn: { _ in })
        XCTAssertEqual(differentCell.exitCode, 1, "a dismissal recorded under a different cell must not suppress this one")
    }
    ...
    func testExpiredDismissalResurfacesThroughDiff() throws {
        ...
        let baseline = DiffCommand.evaluate(outcome: outcome, ollamaModel: "modelA", dismissalsPath: url.path, warn: { _ in })
        ...
        ledger.record(finding, reason: "temporary", recordedAt: Date().addingTimeInterval(-3600), ttl: 60)
        try ledger.save(to: url)

        let evaluation = DiffCommand.evaluate(outcome: outcome, ollamaModel: "modelA", dismissalsPath: url.path, warn: { _ in })
    ...
    func testChangedOutputsChangeSignatureAndAreNotSuppressed() throws {
        ...
        let baseline = DiffCommand.evaluate(outcome: outcomeOriginal, ollamaModel: "modelA", dismissalsPath: url.path, warn: { _ in })
        let cell = try extractField("cell", from: baseline.report)
        let signature = try extractField("signature", from: baseline.report)

        var ledger = DismissalsLedger()
        ledger.record(DismissedFinding(cell: cell, signature: signature), reason: "confirmed", recordedAt: Date(), ttl: 3600)
        try ledger.save(to: url)

        // Sanity: the original divergence is indeed suppressed now.
        let suppressedOriginal = DiffCommand.evaluate(outcome: outcomeOriginal, ollamaModel: "modelA", dismissalsPath: url.path, warn: { _ in })
        ...
        let outcomeChanged = try genuineDivergenceOutcome(ollamaOutput: "one", llamaOutput: "a totally different completion")
        let changed = DiffCommand.evaluate(outcome: outcomeChanged, ollamaModel: "modelA", dismissalsPath: url.path, warn: { _ in })
        XCTAssertEqual(changed.exitCode, 1, "a changed divergence signature must never be silently absorbed into an old dismissal")
    ...
    func testIdenticalOutcomeExitsZeroRegardlessOfDismissalsFlag() {
        let tokens = [1, 2, 3]
        let runs = (0..<2).map { rawRun(backend: "ollama", model: "m", promptSha: "x", output: "same", inputTokens: tokens, repeatIndex: $0) }
        let llamaRuns = (0..<2).map { rawRun(backend: "llama.cpp", model: "m.gguf", promptSha: "x", output: "same", inputTokens: tokens, repeatIndex: $0) }
        ...
        let identicalOutcome = DifferentialOutcome(promptSha256: "x", ollama: report, llama: llamaReport, comparison: comparison)

        let evaluation = DiffCommand.evaluate(outcome: identicalOutcome, ollamaModel: "m", dismissalsPath: ledgerURL().path, warn: { _ in })
        XCTAssertEqual(evaluation.exitCode, 0)
        XCTAssertFalse(evaluation.report.contains("Dismissal"), "the dismissal section is only relevant to genuineDivergence")
    }
}
