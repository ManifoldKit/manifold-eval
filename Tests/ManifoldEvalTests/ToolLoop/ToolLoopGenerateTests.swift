...
final class ToolLoopGenerateTests: XCTestCase {
  ...
  private static func perfectEpisode(
    ...
    events.append(
      .result(content: scriptedResult(toolLoopCase, tool: firstTool, arguments: firstArguments)))

    if let chained = toolLoopCase.expect.chainedCall {
      let chainedArgs = "{\"\(chained.argumentKey)\":\"\(chained.expectedValue)\"}"
      events.append(
        .call(
          name: chained.toolName,
          arguments: [chained.argumentKey: chained.expectedValue].isEmpty ? "{}" : chainedArgs))
      events.append(
        .result(
          content: scriptedResult(
