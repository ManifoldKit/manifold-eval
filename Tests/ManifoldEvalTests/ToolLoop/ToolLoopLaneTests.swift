...
final class ToolLoopLaneTests: XCTestCase {
  ...
  func testBuiltinCorpusHasUniqueIDsAndWellFormedExpectations() throws {
    ...
    for toolLoopCase in cases {
      ...
      if let first = toolLoopCase.expect.firstCall {
        XCTAssertTrue(
          toolNames.contains(first.toolName),
          "\(toolLoopCase.id): firstCall references unknown tool")
      }
      if let chained = toolLoopCase.expect.chainedCall {
        XCTAssertTrue(
          toolNames.contains(chained.toolName),
          "\(toolLoopCase.id): chainedCall references unknown tool")
  ...
  func testExpectationLessFileCaseIsRejectedAtLoad() throws {
    let json =
      #"{"id":"vacuous","userPrompt":"p","tools":[{"name":"t","description":"d","parameters":{"type":"object"},"script":{"result":"{}"}}],"expect":{"finalAnswerMustContain":[]}}"#
