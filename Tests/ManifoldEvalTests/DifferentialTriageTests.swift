...
final class DifferentialTriageTests: XCTestCase {
  ...
  func testRawRunCodableRoundTrip() throws {
    let original = run(
      backend: "llama.cpp", quant: "Q4_K_M", inputTokens: [128000, 1, 2, 3], output: "hi")
  ...
  func testBOSExplicitStripsLeadingBOSOnly() {
    ...
    XCTAssertTrue(
      BOSNormalizer.streamsMatch(
        [128000, 1, 2, 3], [1, 2, 3], normalization: .explicit(bosID: 128000)))
    // Both carry a BOS → still matches (both normalised to no-BOS).
    XCTAssertTrue(
      BOSNormalizer.streamsMatch(
        [128000, 1, 2, 3], [128000, 1, 2, 3], normalization: .explicit(bosID: 128000)))
  }
  ...
  func testBOSExplicitRealDifferenceIsNotStripped() {
    ...
    XCTAssertFalse(
      BOSNormalizer.streamsMatch(
        [128000, 1, 2, 99], [1, 2, 3], normalization: .explicit(bosID: 128000)))
  }

  func testBOSAutoDetectMatchesBOSOnlyDifference() {
    XCTAssertTrue(
      BOSNormalizer.streamsMatch([128000, 1, 2, 3], [1, 2, 3], normalization: .autoDetect))
  ...
  func testBOSAutoDetectRejectsRealDifference() {
    ...
    XCTAssertFalse(
      BOSNormalizer.streamsMatch([128000, 1, 2, 99], [1, 2, 3], normalization: .autoDetect))
  ...
  func testIdentical() {
    ...
    XCTAssertEqual(
      DivergenceTriage.classify(a, b, aIsDeterministic: true, bIsDeterministic: true), .identical)
  }
  ...
  func testPromptDivergenceWinsOverEveryOtherConfound() {
    ...
    XCTAssertEqual(
      DivergenceTriage.classify(a, b, aIsDeterministic: false, bIsDeterministic: false),
      .promptDivergence)
  }
  ...
  func testSamplerMismatchOutranksDegenerateRepetitionShape() {
    ...
    let unit = "loop "
    let a = run(
      promptSha: "p", output: String(repeating: unit, count: 8),
      sampler: SamplerConfig(temperature: 0.0))
    let b = run(
      promptSha: "p", output: String(repeating: unit, count: 3),
  ...
  func testIndeterminateOutranksDegenerateRepetitionShape() {
    ...
    XCTAssertEqual(
      DivergenceTriage.classify(
        a, b, aIsDeterministic: true, bIsDeterministic: true,
  ...
  func testRepeatingUnitNilForTextTooShortToRepeatTwice() {
    ...
    XCTAssertNil(
      DegenerateRepetition.repeatingUnit(of: "ab"), "a period must repeat at least twice")
  }

  func testIsRepetitionLengthMismatchRequiresIdenticalUnit() {
    XCTAssertTrue(
      DegenerateRepetition.isRepetitionLengthMismatch(
  ...
  func testCompareSingleRepeatDifferingOutputsIsIndeterminate() {
    ...
    XCTAssertEqual(
      record?.divergence, .indeterminate,
      "single-repeat differing outputs → indeterminate, not genuine")
  }
  ...
  func testGenuineDivergenceRequiresSamplerAgreement() {
    ...
    let a = run(
      promptSha: "p", output: "x", sampler: SamplerConfig(temperature: 0.0, repeatPenalty: 1.1))
    let b = run(
      promptSha: "p", output: "y", sampler: SamplerConfig(temperature: 0.0, repeatPenalty: 1.1))
    XCTAssertEqual(
      DivergenceTriage.classify(a, b, aIsDeterministic: true, bIsDeterministic: true),
      .genuineDivergence
    )
  ...
  func testDeterminismVariant() {
    ...
    let report = DeterminismReport(runs: [
      run(output: "cold"), run(output: "warm"), run(output: "warm"),
    ])
  ...
  func testRepresentativeFallsBackToFirstWhenAllDistinct() {
    ...
    XCTAssertEqual(
      report.representative?.output, "a", "all-distinct ties resolve to the earliest run")
  }
  ...
  func testCompareHonorsCohortOverride() {
    ...
    let llama = DeterminismReport(runs: [
      run(backend: "llama.cpp", quant: "Q4_K_M", promptSha: "p", output: "x")
    ])
  ...
  func testReportIsDeterministicAndCarriesVerdict() {
    ...
    let outcome = DifferentialOutcome(
      promptSha256: "p", ollama: ollama, llama: llama, comparison: record)
  ...
  func testReportFlagsOllamaOnlyAsControl() {
    ...
    let outcome = DifferentialOutcome(
      promptSha256: "p", ollama: ollama, llama: nil, comparison: nil)
    let report = DivergenceReport.render(outcome)
    XCTAssertTrue(
      report.contains("determinism control"), "Ollama-only run is a control, not a comparison")
  }
}
