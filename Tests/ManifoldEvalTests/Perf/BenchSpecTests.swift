...
final class BenchSpecTests: XCTestCase {
  ...
  private func makeSpec(
    ...
    BenchSpec(
      ...
      lanes: [
        .init(
          name: "ollama", transport: .httpOllama, endpoint: "http://localhost:11434",
          model: "llama3.1-8b", quant: "Q4_K_M"),
        .init(
          name: "omlx", transport: .httpOpenAI, endpoint: "http://127.0.0.1:8000",
          model: "Meta-Llama-3.1-8B-Instruct-4bit", quant: "4bit", apiKeyEnv: "OMLX_API_KEY"),
      ]
    )
  }
  ...
  func testSpecHashIgnoresLaneIdentity() throws {
    ...
    let differentLanesSpec = BenchSpec(
      ...
      lanes: [
        .init(
          name: "solo-lane", transport: .httpOllama, endpoint: "http://elsewhere:1234", model: "x",
          quant: "y")
      ]
    )
  ...
  func testSpecHashChangesWithPrompt() throws {
    XCTAssertNotEqual(
      try makeSpec(prompt: "2 + 2 =").specHash, try makeSpec(prompt: "3 + 3 =").specHash)
  }
