...
final class BenchResultSchemaTests: XCTestCase {
  ...
  func testPublicationThresholds() {
    XCTAssertFalse(
      makeResult(
        ttftMsPerRun: Array(repeating: 1, count: 5), tpsPerRun: Array(repeating: 1, count: 5)
      ).publishesP90)
    XCTAssertTrue(
      makeResult(
        ttftMsPerRun: Array(repeating: 1, count: 20), tpsPerRun: Array(repeating: 1, count: 20)
      ).publishesP90)
    XCTAssertFalse(
      makeResult(
        ttftMsPerRun: Array(repeating: 1, count: 30), tpsPerRun: Array(repeating: 1, count: 30)
      ).publishesP99)
    XCTAssertTrue(
      makeResult(
        ttftMsPerRun: Array(repeating: 1, count: 100), tpsPerRun: Array(repeating: 1, count: 100)
      ).publishesP99)
  }
}
