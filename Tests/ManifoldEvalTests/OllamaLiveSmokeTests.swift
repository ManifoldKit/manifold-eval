...
final class OllamaLiveSmokeTests: XCTestCase {
  ...
  private var model: String {
    ProcessInfo.processInfo.environment["OLLAMA_MODEL"] ?? "llama3.1-8b:latest"
  }
  ...
  func testTriageClassifiesGenuineDivergenceLive() async throws {
    ...
    let a = try await driver.run(
      model: model, prompt: "Say apple.", sampler: .greedy, repeatIndex: 0)
    let b = try await driver.run(
      model: model, prompt: "Say orange.", sampler: .greedy, repeatIndex: 0)
    let verdict = DivergenceTriage.classify(a, b, aIsDeterministic: true, bIsDeterministic: true)
    XCTAssertEqual(
      verdict, .promptDivergence, "different prompt bytes → control fails (promptDivergence)")
  }
}
