...
final class AutomationClaimsTests: XCTestCase {
    ...
    private func repoRoot() throws -> URL {
        ...
        guard FileManager.default.fileExists(atPath: root.appendingPathComponent("Package.swift").path) else {
            throw XCTSkip("Could not locate repo root from #filePath (\(root.path)) — source tree unavailable")
        }
        return root
    }
    ...
    private func rotGuardCron() throws -> String {
        ...
        guard let cron = firstCapture(in: yaml, pattern: #"-\s*cron:\s*"([^"]+)""#) else {
            XCTFail("rot-guard.yml has no `- cron: \"...\"` line — it is no longer scheduled. "
    ...
    private static let workflowFiles = ["ci.yml", "rot-guard.yml", "core-bump.yml", "release-please.yml"]
    ...
    private func declaredTriggerKeys(inWorkflow file: String) throws -> Set<String> {
        let lines = try read(".github/workflows/\(file)").split(separator: "\n", omittingEmptySubsequences: false)
    ...
    func testTriggerVocabularyCoversEveryWorkflow() throws {
        for file in Self.workflowFiles {
            ...
            XCTAssertFalse(declared.isEmpty, "No `on:` keys parsed from \(file) — parser assumption broken")
