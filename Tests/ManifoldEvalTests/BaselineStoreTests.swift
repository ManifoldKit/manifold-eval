...
final class BaselineStoreTests: XCTestCase {
  ...
  func testRoundTripWriteThenReadIsStable() throws {
    ...
    let store = BaselineStore.empty.updated(
      with: manifest.observations, timestamp: Date(timeIntervalSince1970: 1_000))
  ...
  func testReWriteOfSameContentIsByteIdentical() throws {
    ...
    XCTAssertEqual(
      dataA, dataB, "re-writing identical logical content must diff cleanly (byte-identical)")
  }
  ...
  func testUpdatePreservesRowsOutsideCurrentRunScope() throws {
    ...
    let seeded = BaselineStore(rows: [
      BaselineRow(
        key: untouchedKey,
        entry: BaselineEntry(
          score: 0.5, bytesHash: "abc", divergenceClass: "pass",
          timestamp: Date(timeIntervalSince1970: 1), coreCommit: "deadbeef")
      )
    ])

    let manifest = try BaselineCollector.build(from: [record()])
    let updated = seeded.updated(
      with: manifest.observations, timestamp: Date(timeIntervalSince1970: 500))

    XCTAssertEqual(
      updated.byKey[untouchedKey]?.score, 0.5,
      "a cell outside this run's scope must survive an --update untouched")
  ...
  func testMixedCoreCommitsInCurrentRunIsRejected() {
    XCTAssertThrowsError(
      try BaselineCollector.build(from: [
  ...
  func testLoadRejectsDuplicateCellKeyInsteadOfCrashing() throws {
    ...
    let entryA = BaselineEntry(
      score: 0.9, bytesHash: "aaa", divergenceClass: "pass",
      timestamp: Date(timeIntervalSince1970: 1), coreCommit: "4461529f")
    let entryB = BaselineEntry(
      score: 0.4, bytesHash: "bbb", divergenceClass: "fail",
      timestamp: Date(timeIntervalSince1970: 2), coreCommit: "4461529f")
  ...
  func testLoadRejectsBaselineFileSpanningMultipleCoreCommits() throws {
    let entryA = BaselineEntry(
      score: 0.9, bytesHash: "aaa", divergenceClass: "pass",
      timestamp: Date(timeIntervalSince1970: 1), coreCommit: "aaaa")
    let entryB = BaselineEntry(
      score: 0.4, bytesHash: "bbb", divergenceClass: "fail",
      timestamp: Date(timeIntervalSince1970: 2), coreCommit: "bbbb")
  ...
  func testBaselineAtOneCommitVsCurrentRunAtDifferentCommitStillComparesCleanly() throws {
    ...
    let baseline = BaselineStore.empty.updated(
      with: baselineManifest.observations, timestamp: Date(timeIntervalSince1970: 1))
    ...
    XCTAssertEqual(
      changes.count, 1,
      "a baseline@commitA vs current@commitB diff must still report movement — that comparison is the entire point of the loop"
    )
    guard
      case .scoreChanged(let previous, let current, _)? = changes.first?.movements.first(where: {
  ...
  func testScoreChangeBeyondThresholdIsDetected() throws {
    ...
    let baseline = BaselineStore.empty.updated(
      with: baselineManifest.observations, timestamp: Date())

    let currentManifest = try BaselineCollector.build(from: [record(f1: 0.5)])
    let changes = BaselineRotGuard.detectMovements(
      current: currentManifest.observations, baseline: baseline, scoreThreshold: 0.05)

    XCTAssertEqual(changes.count, 1)
    guard
      case .scoreChanged(let previous, let current, let delta)? = changes.first?.movements.first(
        where: {
          if case .scoreChanged = $0 { return true } else { return false }
        })
    else { return XCTFail("expected a scoreChanged movement") }
  ...
  func testDivergenceClassFlipIsDetected() throws {
    ...
    XCTAssertEqual(changes.count, 1)
    let flip = changes[0].movements.first {
      if case .divergenceClassFlipped = $0 { return true } else { return false }
    }
  ...
  func testBytesHashChangeIsDetectedEvenWithoutScoreMovement() throws {
    let baselineManifest = try BaselineCollector.build(from: [
      record(transcriptRef: "ollama/run-a.jsonl")
    ])
    let baseline = BaselineStore.empty.updated(
      with: baselineManifest.observations, timestamp: Date())
    ...
    let currentManifest = try BaselineCollector.build(from: [
      record(transcriptRef: "ollama/run-b.jsonl")
    ])
    let changes = BaselineRotGuard.detectMovements(
      current: currentManifest.observations, baseline: baseline, scoreThreshold: 0.05)

    XCTAssertEqual(changes.count, 1)
    let hashChange = changes[0].movements.first {
      if case .bytesHashChanged = $0 { return true } else { return false }
    }
    XCTAssertNotNil(
      hashChange,
      "a transcriptRef change must surface as a bytesHash movement even though score/verdict are identical"
    )

    let scoreChange = changes[0].movements.first {
      if case .scoreChanged = $0 { return true } else { return false }
    }
  ...
  func testRotGuardTripsOnStaleBaseline() {
    ...
    let baseline = BaselineStore(rows: [
      BaselineRow(
        key: key(),
        entry: BaselineEntry(
          score: 1.0, bytesHash: "abc", divergenceClass: "pass", timestamp: old,
          coreCommit: "4461529f"))
    ])

    let verdict = BaselineRotGuard.evaluate(
      manifestKeys: [key()], baseline: baseline, maxAge: 3600,
  ...
  func testRotGuardDoesNotTripWithinMaxAge() {
    ...
    let baseline = BaselineStore(rows: [
      BaselineRow(
        key: key(),
        entry: BaselineEntry(
          score: 1.0, bytesHash: "abc", divergenceClass: "pass", timestamp: recent,
          coreCommit: "4461529f"))
    ])
  ...
  func testEmptyBaselineIsNotStale() {
    let verdict = BaselineRotGuard.evaluate(
      manifestKeys: [], baseline: .empty, maxAge: 3600, now: Date())
    XCTAssertFalse(
      verdict.isStale, "an empty (never-written) baseline is the pre-first-update state, not rot")
    XCTAssertFalse(verdict.tripped)
  }
  ...
  func testRotGuardTripsOnManifestShrink() {
    ...
    let baseline = BaselineStore(rows: [
      BaselineRow(
        key: vanished,
        entry: BaselineEntry(
          score: 1.0, bytesHash: "abc", divergenceClass: "pass", timestamp: Date(),
          coreCommit: "4461529f"))
    ])

    // The current run's manifest doesn't mention `vanished` at all.
    let verdict = BaselineRotGuard.evaluate(
      manifestKeys: [key()], baseline: baseline, maxAge: 999_999, now: Date())
  ...
  func testNotMeasuredCellPresentInManifestDoesNotTripShrinkOrRegression() throws {
    ...
    XCTAssertTrue(
      currentManifest.observations.isEmpty, "a wholly-notMeasured cell must produce no observation")
    XCTAssertEqual(
      currentManifest.manifestKeys, [key()], "but it must still be present in the manifest")
    ...
    XCTAssertTrue(
      rotVerdict.shrunkCells.isEmpty, "a notMeasured-but-present cell must never read as shrink")
    ...
    XCTAssertTrue(
      changes.isEmpty,
      "a notMeasured cell has no observation to compare — it must never read as a regression")
