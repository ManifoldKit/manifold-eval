...
final class DismissalsLedgerTests: XCTestCase {
  ...
  func testLiveDismissalSuppresses() {
    ...
    let now = Date()
    ledger.record(
      finding, reason: "confirmed by-design: renderer whitespace", recordedAt: now, ttl: 3600)
  ...
  func testExpiredDismissalResurfaces() {
    ...
    XCTAssertEqual(
      surfaced, [finding], "an expired dismissal must resurface the cell — expiry forces a re-look")
  }

  func testChangedSignatureDoesNotSuppress() {
    var ledger = DismissalsLedger()
    let dismissed = DismissedFinding(
      cell: cell, signature: signature(for: "original divergence bytes"))
    let now = Date()
    ...
    let changed = DismissedFinding(
      cell: cell, signature: signature(for: "a completely different divergence"))
    let surfaced = ledger.suppress([changed], asOf: now.addingTimeInterval(60))
    XCTAssertEqual(
      surfaced, [changed], "a changed divergence-signature is new signal and must not be suppressed"
    )
  }
  ...
  func testPruneAtExactExpiryBoundaryTreatsAsExpired() {
    ...
    XCTAssertFalse(
      ledger.entries[0].isLive(asOf: expiresAt),
      "expiry boundary itself must read as expired, not live")
  }
  ...
  func testLedgerJSONRoundTripsStably() throws {
    ...
    let url = FileManager.default.temporaryDirectory.appendingPathComponent(
      "dismissals-roundtrip-\(UUID().uuidString).json")
    ...
    XCTAssertEqual(
      firstData, secondData, "re-encoding an unchanged ledger must produce byte-identical JSON")
  ...
  func testGenuineSubMillisecondJitterSurvivesRoundTripAtMillisecondPrecision() throws {
    ...
    let url = FileManager.default.temporaryDirectory.appendingPathComponent(
      "dismissals-submillis-\(UUID().uuidString).json")
    defer { try? FileManager.default.removeItem(at: url) }
    try ledger.save(to: url)
    let reloaded = try DismissalsLedger.load(from: url)
  ...
  func testLoadMissingFileYieldsEmptyLedger() throws {
    let url = FileManager.default.temporaryDirectory.appendingPathComponent(
      "does-not-exist-\(UUID().uuidString).json")
  ...
  func testSignatureChangesWithDivergenceClassOrBytes() {
    ...
    let b = DivergenceSignature.compute(
      divergenceClass: "degenerateRepetitionLengthMismatch", differingText: "x")
  ...
  func testSignatureIsDeterministic() {
    let a = DivergenceSignature.compute(
      divergenceClass: "genuineDivergence", differingText: "same bytes")
    let b = DivergenceSignature.compute(
      divergenceClass: "genuineDivergence", differingText: "same bytes")
    XCTAssertEqual(a, b)
  }
