...
final class RegressionCrossQuantLiveTests: XCTestCase {
  ...
  private func skipUnlessInstalled(_ tags: [String], at url: URL) async throws {
    ...
    if !missing.isEmpty {
      throw XCTSkip(
        "missing Ollama model(s): \(missing.joined(separator: ", ")). "
  ...
  private var reDrivenQuant: String {
    env("REGRESS_REDRIVEN_MODEL", "qwen2.5:0.5b-instruct-q4_K_M")
  }
    ...
    let outcome = try await RegressionRunner.run(
      ...
      captureBaseline: {
        try await driver.run(
          model: self.baselineQuant, prompt: self.prompt, sampler: .greedy, repeatIndex: 0)
      },
      captureReDriven: {
        try await driver.run(
          model: self.baselineQuant, prompt: self.prompt, sampler: .greedy, repeatIndex: 1)
      }
    )
  ...
  func testCrossQuantVerdictTracksScores() async throws {
    ...
    let outcome = try await RegressionRunner.run(
      ...
      captureReDriven: {
        try await driver.run(
          model: self.reDrivenQuant, prompt: self.prompt, sampler: .greedy, repeatIndex: 0)
      }
    )
    ...
    print(
      "[cross-quant] \(baselineQuant)=\(baselineScore) vs \(reDrivenQuant)=\(reDrivenScore) "
