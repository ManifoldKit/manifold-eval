...
enum DiffCommand {
    ...
    while index < args.count {
      ...
      case "--repeat-penalty":
        ...
        guard let d = Double(raw) else {
          die("--repeat-penalty requires a number, got '\(raw)'", 2)
        }
  ...
  static func run(
    ...
    if let promptFile {
      ...
      do {
        ...
        prompt = try PromptRendering.render(
          messages: messages, ggufURL: URL(fileURLWithPath: templateGGUF))
      } catch {
        die("\(error)", 1)
      }
    } else {
  ...
  static func evaluate(
    ...
    case .genuineDivergence:
      ...
      let cell = [ollamaModel, repA.backend, repB.backend, outcome.promptSha256].joined(
        separator: "|")
      ...
      report +=
        "--signature '\(signature)' --reason <text> --ttl <seconds> --ledger \(dismissalsPath)` to suppress\n"
      return Evaluation(report: report, exitCode: 1)
    }
  }
}
