...
enum RegressCommand {
  ...
  static func run(
    ...
    case "ollama":
      ...
      captureBaseline = {
        try await driver.run(model: baselineModel, prompt: prompt, sampler: sampler, repeatIndex: 0)
      }
      captureReDriven = {
        try await driver.run(model: reDrivenModel, prompt: prompt, sampler: sampler, repeatIndex: 0)
      }
    case "llama":
      ...
      captureBaseline = {
        try await driver.run(
          modelArg: baselineModel, promptFile: promptFileURL, sampler: sampler, repeatIndex: 0)
      }
      captureReDriven = {
        try await driver.run(
          modelArg: reDrivenModel, promptFile: promptFileURL, sampler: sampler, repeatIndex: 0)
      }
    default:
    ...
    warn(
      "regress: baseline='\(baselineModel)' vs re-driven='\(reDrivenModel)' on \(backend) (greedy temp=\(temperature))…"
    )
