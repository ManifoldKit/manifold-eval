...
enum BaselineCommand {
    ...
    static func run(
        ...
        // --- Compare mode ---
        guard !existingBaseline.rows.isEmpty || FileManager.default.fileExists(atPath: baselinePath.path) else {
    ...
    private static func renderReport(changes: [CellChange], rotVerdict: RotGuardVerdict) -> String {
        ...
        if rotVerdict.tripped {
            ...
            if rotVerdict.isStale {
                let newest = rotVerdict.newestBaselineTimestamp.map { ISO8601DateFormatter().string(from: $0) } ?? "n/a"
                lines.append("- baseline is stale: newest row is \(newest), max age is \(Int(rotVerdict.maxAge / 3600))h")
            }
            if !rotVerdict.shrunkCells.isEmpty {
                lines.append("- cell manifest shrank — \(rotVerdict.shrunkCells.count) cell(s) present in the baseline are absent from this run's manifest entirely:")
        ...
        if changes.isEmpty {
            ...
            for change in changes {
                lines.append("- **\(change.key.model) / \(change.key.quant) / \(change.key.backend) / \(change.key.renderer)**")
