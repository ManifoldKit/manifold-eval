...
enum DismissalsCommand {
    ...
    static func runDismiss(
        ...
        print("dismissed cell='\(cell)' signature='\(signature)' until \(ISO8601DateFormatter().string(from: expiresAt))")
    }
    ...
    static func runDismissals(
        ...
        if prune {
            ...
            for entry in removed {
                warn("pruned cell='\(entry.finding.cell)' signature='\(entry.finding.signature)' (expired \(ISO8601DateFormatter().string(from: entry.expiresAt)))")
            }
            print("pruned \(removed.count) expired entr\(removed.count == 1 ? "y" : "ies"); \(ledger.count) remaining")
            return
        }
