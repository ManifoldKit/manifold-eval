...
public struct RotGuardVerdict: Sendable, Equatable {
    ...
    public init(shrunkCells: [CellKey], isStale: Bool, newestBaselineTimestamp: Date?, maxAge: TimeInterval) {
    ...
    public static func detectMovements(
        ...
        for (key, observation) in current {
            ...
            if abs(delta) > scoreThreshold {
                movements.append(.scoreChanged(previous: previous.score, current: observation.score, delta: delta))
            }
            if previous.divergenceClass != observation.divergenceClass {
                movements.append(.divergenceClassFlipped(previous: previous.divergenceClass, current: observation.divergenceClass))
            }
            if previous.bytesHash != observation.bytesHash {
                movements.append(.bytesHashChanged(previous: previous.bytesHash, current: observation.bytesHash))
            }
