...
public struct TriageBrief: Sendable, Equatable {
  ...
  static func confidenceAssessment(
    ...
    if !sameBytesOK {
      return (
        .low,
        ["same-bytes control UNMET — prompt hashes differ; the comparison itself may be invalid"]
      )
    }
    if !determinismOK {
      return (
        .low,
        [
          "determinism control UNMET — a leg had fewer than 2 repeats, so its reproducibility is unknown"
        ]
      )
    }
    switch divergence {
    case .samplerNondeterminism:
      return (
        .medium,
        [
          "a leg was observed non-reproducible across its own repeats — the output comparison is noisy"
        ]
      )
    case .degenerateRepetitionLengthMismatch:
      return (
        .medium,
        [
          "outputs reduce to the same repeating unit at different lengths — a stopping-length artifact, not necessarily a content difference"
        ]
      )
    case .identical, .promptDivergence, .tokenizerDivergence, .samplerMismatch, .indeterminate,
      .genuineDivergence:
      return (
        .high,
        [
          "both the same-bytes and determinism controls are satisfied; the verdict rests on deterministic evidence"
        ]
      )
    }
  }
  ...
  public func render() -> String {
    ...
    if let diverging {
      out +=
        "- common prefix: \(diverging.commonPrefixLength) bytes, common suffix: \(diverging.commonSuffixLength) bytes\n"
      out +=
        "- leg A differing segment (\(diverging.aDiffering.utf8.count) bytes): \(fence(diverging.aDiffering))\n"
      out +=
        "- leg B differing segment (\(diverging.bDiffering.utf8.count) bytes): \(fence(diverging.bDiffering))\n"
    } else {
