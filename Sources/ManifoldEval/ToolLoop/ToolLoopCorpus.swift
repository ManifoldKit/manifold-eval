...
public enum ToolLoopCorpus {
  ...
  public static func load(path: String?) throws -> [ToolLoopCase] {
    ...
    for rawLine in String(decoding: data, as: UTF8.self).split(whereSeparator: \.isNewline) {
      ...
      guard
        loaded.expect.firstCall != nil
          || loaded.expect.chainedCall != nil
          || !loaded.expect.finalAnswerMustContain.isEmpty
      else {
  ...
  public static let builtin: [ToolLoopCase] = [
    ...
    ToolLoopCase(
      id: "thread_gate_1",
      userPrompt:
        "Which gate does flight QF123 depart from? Use the available tools to check, then answer.",
      tools: [
        ScriptedToolSpec(
          name: "get_flight_status",
          description: "Look up the live status record for a flight by its flight number.",
          parameters: objectSchema(
            ["flight": "The flight number, e.g. QF123"], required: ["flight"]),
          script: ToolScript(result: #"{"flight":"QF123","status":"delayed","gate":"K97"}"#)
        )
      ],
      expect: ToolLoopExpectations(
        firstCall: ToolLoopExpectedCall(
          toolName: "get_flight_status", arguments: ["flight": "QF123"]),
    ...
    ToolLoopCase(
      id: "thread_depot_1",
      userPrompt:
        "Where is parcel PX-58731 right now? Use the available tools to check, then answer.",
      tools: [
        ScriptedToolSpec(
          name: "get_parcel_status",
          description: "Look up the current location record for a parcel by tracking id.",
          parameters: objectSchema(
            ["tracking_id": "The parcel tracking id"], required: ["tracking_id"]),
          script: ToolScript(
            result: #"{"tracking_id":"PX-58731","location":"DEPOT-4Q2","last_scan":"inbound"}"#)
        )
      ],
      expect: ToolLoopExpectations(
        firstCall: ToolLoopExpectedCall(
          toolName: "get_parcel_status", arguments: ["tracking_id": "PX-58731"]),
    ...
    ToolLoopCase(
      id: "thread_carrier_1",
      userPrompt:
        "Which carrier is delivering order 884213? Use the available tools to check, then answer.",
      tools: [
        ScriptedToolSpec(
          name: "get_order",
          ...
          script: ToolScript(
            result: #"{"order_id":"884213","carrier":"Skyfreight","eta_days":"9"}"#)
        )
      ],
      expect: ToolLoopExpectations(
    ...
    ToolLoopCase(
      ...
      userPrompt:
        "What are the current weather conditions in Cairo? Use the available tools to check, then answer with what the tool reports.",
      tools: [
        ScriptedToolSpec(
      ...
      expect: ToolLoopExpectations(
        firstCall: ToolLoopExpectedCall(
          toolName: "get_current_weather", arguments: ["city": "Cairo"]),
    ...
    ToolLoopCase(
      id: "chain_account_1",
      userPrompt:
        "What is the current balance for the customer with email sam@example.com? First look up their account, then fetch the balance, then answer.",
      tools: [
        ScriptedToolSpec(
          name: "lookup_account",
        ...
        ScriptedToolSpec(
          ...
          parameters: objectSchema(
            ["account_id": "The account id, e.g. ACC-12345"], required: ["account_id"]),
          script: ToolScript(
      ...
      expect: ToolLoopExpectations(
        firstCall: ToolLoopExpectedCall(
          toolName: "lookup_account", arguments: ["email": "sam@example.com"]),
    ...
    ToolLoopCase(
      id: "chain_booking_1",
      userPrompt:
        "Which seat is Rory Ford booked in? First find the booking reference, then fetch the seat for that booking, then answer.",
      tools: [
        ScriptedToolSpec(
          name: "find_booking",
        ...
        ScriptedToolSpec(
          ...
          parameters: objectSchema(
            ["booking_ref": "The booking reference, e.g. ABC-1234"], required: ["booking_ref"]),
          script: ToolScript(
    ...
    ToolLoopCase(
      id: "chain_forecast_1",
      userPrompt:
        "What is tomorrow's forecast where user rory is located? First look up the user's city, then fetch that city's forecast, then answer.",
      tools: [
        ScriptedToolSpec(
    ...
    ToolLoopCase(
      id: "multi_quote_1",
      userPrompt:
        "Get quotes for the stock symbols NVAX and BLZR, and report both prices in your answer.",
      tools: [
        ScriptedToolSpec(
          name: "get_stock_quote",
          description: "Get the latest trade price for a stock by ticker symbol.",
          parameters: objectSchema(
            ["symbol": "The ticker symbol, e.g. NVAX"], required: ["symbol"]),
          script: ToolScript(
